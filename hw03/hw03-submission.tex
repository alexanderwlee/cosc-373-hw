...
\item Each node then does the following:
    ...
    (Correctness) The protocol above uses the LE/BFS/SSP protocol discussed in
    class to elect a leader, find a BFS tree, and notify the leader of the
    distance from the leader to the furthest node/nodes in O(D) rounds. The
    leader then knows that it will take "Dist" (which can be at most D) rounds to
    get from the leader to the furthest away node/nodes. Therefore, the leader
    decides to wait "Dist" rounds before resetting and then lets its neighbors
    know to wait one less round and then those nodes let their neighbors know to
    wait one less round and so on. The message that a node receives, tells the
    node how many rounds to wait until resetting or how many rounds it will take
    until we reach the furthest node/nodes. Furthermore, $second\_counter$ and
    $reset\_cntdown$ help each node to keep track of when to reset and
    ...
    node/nodes will then have $second\_counter$ = $reset\_cntdown$ = 0 and 
    ...
    take O(D) rounds. We know that the LE/BFS/SSP algorithm will also take O(D) 
    rounds. Therefore, this protocol will take O(D) rounds.  
    
    (Congestion) We already know that the CONGEST protocol LE/BFS/SSP from class 
    sends less than $O(nlogn)$ bits. Furthermore, the algorithm above has nodes 
